...
- **The translation is pure, the process does the IO.** Everything a test
  should see of the rows goes into the pure `model` library
  (`src/model.lua`); `src/window.lua` reads the threads contract and runs its
  definition with `app.main`. Use the SDK as documented in `docs/sdk.md` and
  the skill in `skills/wippy-window-app/SKILL.md`: the compositor owns the
  frame, the tree is plain tables, every interactive component has a stable
  `id`.
- **Report actual tests and what they saw**: no suite draws this window's
  shot, and a green run is not proof a window looks right.
...
make test           # the harness's suites
