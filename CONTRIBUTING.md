...
tests for public behaviour (the model in `test/src/model_test.lua`, the
entry and the window over the stand-in contract in
`test/src/window_test.lua`), and attach or describe a screenshot when the
window's look changed. Avoid unrelated formatting and
